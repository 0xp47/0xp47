...
def send_discord_rollcall(webhook_url, repo, contributors):
    ...
    if contributors:
        contributors_list = "\n".join([f"- **{name}** ({count} commit(s))" for name, count in sorted(contributors.items(), key=lambda x: x[1], reverse=True)])
        embed["description"] = f"A huge thank you to everyone who contributed to **[{repo}](https://github.com/{repo})** over the last 7 days! 🙌\n\n### Contributors of the week:\n{contributors_list}"
    else:
        embed["description"] = f"No new commits recorded in **[{repo}](https://github.com/{repo})** this week. Let's start building! 🚀"
