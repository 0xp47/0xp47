...
def send_streak_warning(webhook_url, repo):
    ...
    if response.status_code in [200, 204]:
        ...
        print(
            f"Failed to send streak warning: {response.status_code} - {response.text}"
        )
...
def main():
    ...
    if has_commit_today():
        print(
            "🟢 You have already made a commit today! Your contribution streak is safe."
        )
        sys.exit(0)
    ...
    if webhook_url:
        ...
        print(
            "💡 Setup a DISCORD_WEBHOOK_URL environment variable to receive automated alerts."
        )
