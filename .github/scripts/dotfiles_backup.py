...
def backup_vscode():
    ...
    if extensions:
        with open(os.path.join(DOTFILES_DIR, "vscode", "extensions.txt"), "w", encoding="utf-8") as f:
            f.write(extensions)
...
def backup_shell_configs():
    ...
    if system in ["Linux", "Darwin"]:
        for file in [".bashrc", ".zshrc", ".bash_profile", ".profile", ".gitconfig"]:
            ...
            if os.path.exists(path):
                shutil.copy2(path, os.path.join(DOTFILES_DIR, "shell", file.replace(".", "")))
    ...
    # 2. Windows PowerShell Profiles
        ...
        if ps_profile and os.path.exists(ps_profile):
            shutil.copy2(ps_profile, os.path.join(DOTFILES_DIR, "shell", "Microsoft.PowerShell_profile.ps1"))
