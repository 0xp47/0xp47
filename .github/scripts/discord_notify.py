...
def extract_metrics():
    ...
    match = re.search(r'<!-- STATS:START -->\s*```text\s*(.*?)\s*```\s*<!-- STATS:END -->', content, re.DOTALL)
    if not match:
        return None
    ...
    if top_lang: stats['top_lang'] = top_lang.group(1).strip()
    if top_editor: stats['top_editor'] = top_editor.group(1).strip()
    if peak_time: stats['peak_time'] = peak_time.group(1).strip()
    if wakatime_total: stats['wakatime_total'] = wakatime_total.group(1).strip()
    if repos_stars:
...
def send_discord_notification(webhook_url, stats):
    ...
    if stats:
        embed["fields"].extend([
            {"name": "💻 Top Language", "value": stats.get('top_lang', 'Unknown'), "inline": True},
            {"name": "✍️ Top Editor", "value": stats.get('top_editor', 'Unknown'), "inline": True},
            {"name": "⏰ Peak Time", "value": stats.get('peak_time', 'Unknown'), "inline": True},
            {"name": "⏱️ Coding Time (Total)", "value": stats.get('wakatime_total', 'n/a'), "inline": False},
            {"name": "📦 Repositories", "value": stats.get('repos', 'n/a'), "inline": True},
            {"name": "⭐ GitHub Stars", "value": stats.get('stars', '0 stars'), "inline": True}
        ])
    else:
...
def main():
    ...
    if not webhook_url:
        ...
        sys.exit(0) # Exit silently so workflow doesn't fail if webhook is not configured
