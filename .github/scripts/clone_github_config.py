...
def copy_config_to_repo(dest_repo_path):
    ...
    if not os.path.isdir(git_dir):
        print(
            f"❌ Error: '{dest_repo_path}' is not a valid Git repository (missing .git directory)."
        )
        return False
    ...
    items_to_copy = [
        ...
        os.path.join("scripts", "pre_commit_scanner.py"),
        os.path.join("scripts", "setup_pre_commit.py"),
        "labeler.yml",
        "dependabot.yml",
        "ISSUE_TEMPLATE",
    ]
    ...
    # Run the setup script in the destination repository to auto-install the hook
    dest_setup_script = os.path.join(dest_github_dir, "scripts", "setup_pre_commit.py")
    if os.path.exists(dest_setup_script):
        try:
            import subprocess

            subprocess.run(
                [sys.executable, dest_setup_script],
                cwd=dest_repo_path,
                capture_output=True,
                text=True,
            ...
            print(
                "   🔒 Local pre-commit hook automatically installed & active in destination repo."
            )
        except Exception as e:
            print(f"   ⚠️  Failed to auto-install hook in destination: {e}")

    print(
        f"🎉 Configuration propagation complete! Successfully copied {success_count} files/folders.\n"
    )
    return True
...
def scan_and_propagate_sibling_repos():
    ...
    if choice == "q" or not choice:
        ...
        confirm = (
            input("Are you sure you want to copy to ALL sibling repositories? (y/n): ")
            .strip()
            .lower()
        )
        if confirm == "y":
